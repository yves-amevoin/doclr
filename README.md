...
REST API. It turns PDFs, Office files, images and HTML into Markdown, plain
text, HTML, structured JSON or DocTags, by talking to a docling-serve
instance over HTTP.

There is no Python in the loop. `doclr` speaks HTTP with `httr2`, so the R
side has no `reticulate` dependency and no model stack to install.
...
`docling-client`, not of `doclingr`. It is a slim, remote-first wrapper — the
models run on the server, not in your session.
...
Or provision one locally. `doclr_setup()` is explicit and interactive by
design: it never runs on load or attach, and it asks before installing
anything.
...
Docker is on your `PATH`, whether a package manager could install Podman,
whether Python is available for `pip install "docling-serve[ui]"`, and
finally — on a locked-down machine with none of the above — asks for the URL
of a remote server rather than guessing.
...
`doclr_convert()` is the one function most work goes through. It submits the
job asynchronously, polls with a progress spinner, and returns the finished
document:
...
Local files go through the same call — anything that is not an `http(s)` URL
is uploaded:
...
| Function | Endpoint |
|---|---|
| `doclr_convert_source()` | `POST /v1/convert/source` |
| `doclr_convert_file()` | `POST /v1/convert/file` |
| `doclr_convert_source_async()` | `POST /v1/convert/source/async` |
| `doclr_convert_file_async()` | `POST /v1/convert/file/async` |
| `doclr_task_status()` | `GET /v1/status/poll/{task_id}` |
| `doclr_task_result()` | `GET /v1/result/{task_id}` |

Drive the async flow yourself when you want to do something else while the
server works:
...
Every failure is an `rlang` condition with a `doclr_error_*` class, so you
